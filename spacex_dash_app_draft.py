...
def get_pie_chart(entered_site):
    pie_df = spacex_df
    if entered_site == 'ALL':
        fig_pie_all = px.pie(pie_df, values='class', 
        names= 'Launch Site', 
        title='Successful flights per Launch Site')
        return fig_pie_all
    else:
        # return the outcomes piechart for a selected site
        pie_df = spacex_df.loc[spacex_df['Launch Site'] == entered_site]
        #fig = px.pie(pie_df, values='class',
        fig_pie_ftl = px.pie(pie_df, 
        names= 'class', 
...
def get_scatter_plot(entered_site , payload_range):
    ...
    if entered_site == 'ALL':
        fig_scatter_all = px.scatter(scatter_df[mask], x='Payload Mass (kg)', y='class',
        ...
        #low, high = payload_range
        #mask = (scatter_df['Payload Mass (kg)'] > low) & (scatter_df['Payload Mass (kg)'] < high)
        fig_scatter_flt = px.scatter(scatter_df[mask], x='Payload Mass (kg)', y='class',
        size='Payload Mass (kg)',
        symbol='class',
        color='Booster Version Category', 
        title= f"'Correlation between Payload and Success for Site {entered_site}")
        return fig_scatter_flt
